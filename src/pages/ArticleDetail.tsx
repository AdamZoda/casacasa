  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      ...
      <div className="sticky top-[3.75rem] md:top-[4.5rem] z-30 bg-bg-primary/95 backdrop-blur-sm border-b border-border-primary">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-3 sm:py-6 flex items-center justify-between gap-2">
          <button
          ...
          >
            ...
            <span className="text-[10px] sm:text-sm tracking-[0.18em] sm:tracking-widest uppercase">Retour aux articles</span>
          </button>

          <button
            onClick={() => navigate(`/book/${universeId}/${activityId}/${articleId}`)}
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-6 py-2 bg-brand-gold hover:bg-brand-gold/90 text-bg-primary font-semibold text-[11px] sm:text-base rounded-md sm:rounded-lg transition-all"
          >
      ...
      {/* Main Content */}
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-20 sm:py-60 md:py-70">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-12 lg:gap-16">
          ...
          >
            ...
            <div className="relative group">
              {images.length > 0 ? (
                <div className="relative overflow-hidden rounded-md sm:rounded-lg bg-text-primary/5 aspect-square">
                  <img
                  ...
                  {images.length > 1 && (
                    <>
                      ...
                      className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-20 p-2 sm:p-3 rounded-full bg-black/40 hover:bg-black/60 text-white transition-all opacity-0 group-hover:opacity-100"
                      ...
                      className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-20 p-2 sm:p-3 rounded-full bg-black/40 hover:bg-black/60 text-white transition-all opacity-0 group-hover:opacity-100"
          ...
          >
            {/* Breadcrumb */}
            <div className="text-[10px] sm:text-sm text-text-primary/60 uppercase tracking-[0.14em] sm:tracking-widest">
            ...
            {/* Title */}
            <div>
              <h1 className="text-3xl sm:text-5xl md:text-6xl font-serif mb-2 sm:mb-4 leading-tight">{article.title}</h1>
              <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
                <div className="text-xl sm:text-3xl text-brand-gold font-serif">{formatPrice()}</div>
                {article.availabilityCount && (
                  <span className="text-[10px] sm:text-sm text-green-500 bg-green-500/10 px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-md sm:rounded-lg">
            ...
            {article.description && (
              <div className="space-y-2 sm:space-y-4">
                <h2 className="text-base sm:text-lg font-serif text-text-primary">Description</h2>
                <p className="text-text-primary/70 leading-relaxed text-sm sm:text-lg whitespace-pre-line break-words">
                  {article.description}
                </p>
              </div>
            )}
            ...
            {/* Pricing Info */}
            <div className="bg-text-primary/5 border border-border-primary rounded-md sm:rounded-lg p-3 sm:p-6 space-y-3 sm:space-y-4">
              <h3 className="font-serif text-base sm:text-lg">Informations Tarifaires</h3>
              <div className="space-y-2 sm:space-y-3">
                <div className="flex justify-between items-center pb-2 sm:pb-3 border-b border-border-primary/30 text-xs sm:text-base">
                ...
                {article.priceType === "fixed" ? (
                  <div className="flex justify-between items-center pb-2 sm:pb-3 border-b border-border-primary/30 text-xs sm:text-base">
                    ...
                    <span className="font-semibold text-brand-gold text-sm sm:text-lg">
            ...
            {/* Stock Info */}
            {article.availabilityCount && (
              <div className="bg-green-500/10 border border-green-500/20 rounded-md sm:rounded-lg p-3 sm:p-4">
            ...
            {/* CTA Buttons */}
            <div className="pt-4 sm:pt-8 space-y-3 sm:space-y-4 mt-auto">
              <button
                onClick={() => navigate(`/book/${universeId}/${activityId}/${articleId}`)}
                className="w-full px-4 sm:px-8 py-3 sm:py-4 bg-brand-gold hover:bg-brand-gold/90 text-bg-primary font-bold text-sm sm:text-lg rounded-md sm:rounded-lg transition-all hover:shadow-lg hover:shadow-brand-gold/30 flex items-center justify-center gap-2 sm:gap-3"
              >
              ...
              <button
                onClick={() => navigate(`/activity/${universeId}/${activityId}/articles`)}
                className="w-full px-4 sm:px-8 py-3 sm:py-4 bg-text-primary/5 hover:bg-text-primary/10 text-text-primary text-sm sm:text-base font-semibold rounded-md sm:rounded-lg transition-all border border-border-primary"
              >
